# Sync Gmail address from roster CSV via Cloud Run Job.
# Updates employee_hr_details.gmail_address only.
# Does NOT change affiliations, profiles, status/type, or other HR detail fields.
#
# Usage:
#   deploy\sync-gmail-address-prod.cmd --dry-run
#   deploy\sync-gmail-address-prod.cmd
#   deploy\sync-gmail-address-prod.cmd -File "database/imports/社員名簿 (2).csv"
...
$JobName = "employee-sync-gmail-address"
...
Write-Host ""
Write-Host "WARNING: Updates Gmail address only (from roster CSV Googleアドレス)."
Write-Host "         Affiliations, profiles, status/type, and other HR detail fields are NOT changed."
Write-Host "         Only gmail_address is added or updated."
Write-Host "         Rows without Googleアドレス in CSV are skipped."
Write-Host ""
...
$artisanArgs = @("artisan", "employee:sync-gmail-address-from-roster", $containerCsvPath)
if ($DryRun) {
    $artisanArgs += "--dry-run"
}
if ($MatchEmailOnly) {
...
if ($DryRun) {
    ...
    Write-Host ""
    Write-Host "Production Gmail address sync completed."
}
