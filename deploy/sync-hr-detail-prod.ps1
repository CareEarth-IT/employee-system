# Sync employment status and type from roster CSV via Cloud Run Job.
# Updates employee_hr_details.employment_status and employment_type only.
# Does NOT change affiliations, profiles, or other HR detail fields.
#
# Usage:
#   deploy\sync-hr-detail-prod.cmd --dry-run
#   deploy\sync-hr-detail-prod.cmd
#
...
$JobName = "employee-sync-hr-detail"
...
$artisanArgs = @("artisan", "employee:sync-hr-detail-from-roster", $containerCsvPath)
if ($DryRun) {
    $artisanArgs += "--dry-run"
}
if ($MatchEmailOnly) {
...
if ($DryRun) {
    ...
    Write-Host ""
    Write-Host "Production HR detail status/type sync completed."
}
