# Full sync from Airtable roster CSV via Cloud Run Job.
...
Export Airtable employee roster to CSV, then copy to:
...
$artisanArgs = @("artisan", "employee:sync-from-roster", $containerCsvPath)
if ($DryRun) {
    $artisanArgs += "--dry-run"
}
if ($MatchEmailOnly) {
...
if ($DryRun) {
    ...
    Write-Host ""
    Write-Host "Production roster sync completed."
}
