...
#
# Usage:
#   deploy\sync-affiliation-position-prod.cmd --dry-run
#   deploy\sync-affiliation-position-prod.cmd
...
$JobName = "employee-sync-affiliation-position"
...
$artisanArgs = @("artisan", "employee:sync-affiliation-position")
if ($DryRun) {
    $artisanArgs += "--dry-run"
}
...
if ($DryRun) {
    ...
    Write-Host ""
    Write-Host "Production affiliation position sync completed."
}
