# Grant real_estate_app on production Cloud SQL via Cloud Run job (unix socket).
# Same approach as grant-specific-skills-app-prod.ps1 (avoids MySQL 1410 on gcloud sql import).
# Does NOT modify ceemployee database contents.
#
# Usage:
#   deploy\grant-real-estate-app-prod.cmd -RootPassword "your-root-password"
#   deploy\grant-real-estate-app-prod.cmd -RootPassword "..." -SyncPortalPassword
...
$JobName = "employee-grant-real-estate"
...
Write-Host ""
Write-Host "=== Grant real_estate_app (Cloud Run job) ===" -ForegroundColor Cyan
Write-Host "Target DB: real_estate only (ceemployee data unchanged)"
Write-Host ""
...
$grantScriptPath = Join-Path $PSScriptRoot "scripts\grant-real-estate-app.php"
...
if ($SyncPortalPassword) {
    ...
    if ($portalPassword -eq "") {
        throw "real_estate_app password file is empty"
    }
