# Sync company phone from roster CSV via Cloud Run Job.
# Updates employee_hr_details.company_phone only.
# Does NOT change affiliations, profiles, status/type, or other HR detail fields.
#
# Recommended production flow (existing data is preserved):
#   1. deploy\docker-deploy.cmd
#      - deploys the app and runs DB migrations (company_phone column)
#   2. deploy\sync-company-phone-prod.cmd --dry-run
#      - preview phone updates only
#   3. deploy\sync-company-phone-prod.cmd
#      - apply phone numbers from roster CSV
#
# Usage:
#   deploy\sync-company-phone-prod.cmd --dry-run
#   deploy\sync-company-phone-prod.cmd
#
...
$JobName = "employee-sync-company-phone"
...
Write-Host ""
Write-Host "WARNING: Updates company phone only (from roster CSV 電話番号)."
Write-Host "         Affiliations, profiles, status/type, and other HR detail fields are NOT changed."
Write-Host "         Only company_phone is added or updated."
...
$artisanArgs = @("artisan", "employee:sync-company-phone-from-roster", $containerCsvPath)
if ($DryRun) {
    $artisanArgs += "--dry-run"
}
if ($MatchEmailOnly) {
...
if ($DryRun) {
    ...
    Write-Host ""
    Write-Host "Production company phone sync completed."
}
