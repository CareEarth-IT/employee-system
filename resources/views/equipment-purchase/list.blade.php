...
<div class="max-w-6xl mx-auto">
    <div class="mb-4 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold">備品購入 一覧</h1>
            @if ($showsOnlyOwnApplications ?? false)
                <p class="text-base text-slate-600 mt-1">自分が送信した申請のみ表示されます</p>
            @endif
        </div>
        <a href="{{ route('equipment-purchases.index') }}" class="text-base text-blue-600 hover:underline shrink-0">備品購入トップへ</a>
    </div>

    @if (session('success'))
        <div class="mb-4 rounded border border-green-200 bg-green-50 px-4 py-3 text-green-800 text-base">
            {{ session('success') }}
        </div>
    @endif
    ...
    <form method="GET" action="{{ route('equipment-purchases.list') }}" class="bg-white border border-slate-300 rounded-lg p-6 mb-6 space-y-4">
        <div class="flex flex-wrap items-end gap-4">
            <div class="min-w-[10rem] flex-1">
                <label for="keyword" class="block text-base mb-1">キーワード検索</label>
                <input
                    id="keyword"
                    type="text"
                    name="keyword"
            ...
            <div class="min-w-[10rem]">
                <label for="department" class="block text-base mb-1">部門別検索</label>
                <select id="department" name="department" class="w-full rounded border border-slate-300 px-3 py-2 text-base">
            ...
            <div class="min-w-[10rem]">
                <label for="location" class="block text-base mb-1">拠点別検索</label>
                <select id="location" name="location" class="w-full rounded border border-slate-300 px-3 py-2 text-base">
                    <option value="">すべて</option>
        ...
        <div class="flex flex-wrap items-end gap-4">
            <div>
                <label class="block text-base mb-1">期間検索</label>
                <div class="flex items-center gap-2">
                    <input type="date" name="date_from" value="{{ $filters['date_from'] }}" class="rounded border border-slate-300 px-3 py-2 text-base">
                    <span class="text-slate-500">〜</span>
                    <input type="date" name="date_to" value="{{ $filters['date_to'] }}" class="rounded border border-slate-300 px-3 py-2 text-base">
                </div>
            </div>

            <div>
                <label class="block text-base mb-1">金額検索（税込）</label>
                <div class="flex items-center gap-2">
                    <input type="number" name="price_min" min="0" value="{{ $filters['price_min'] }}" placeholder="下限" class="w-28 rounded border border-slate-300 px-3 py-2 text-base">
                    <span class="text-slate-500">〜</span>
                    <input type="number" name="price_max" min="0" value="{{ $filters['price_max'] }}" placeholder="上限" class="w-28 rounded border border-slate-300 px-3 py-2 text-base">
                </div>
            </div>
    ...
    <div class="bg-white border border-slate-300 rounded-lg overflow-hidden mb-6">
        <div class="px-4 py-3 border-b border-slate-300 bg-slate-100 text-base font-semibold text-slate-900">
            結果一覧
        </div>
                ...
                <tbody>
                    @forelse ($applications as $application)
                        <tr @class([
                            ...
                            <td class="px-3 py-2 text-base max-w-[10rem]">{{ Str::limit($application->product_name, 25) }}</td>
                            <td class="px-3 py-2 text-base max-w-[10rem]">{{ Str::limit($application->listMemo(), 25) ?: '—' }}</td>
        ...
        @if ($applications->hasPages())
            <div class="px-4 py-3 border-t border-slate-300 bg-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3 text-base text-slate-600">
        ...
        @else
            <div class="px-4 py-3 border-t border-slate-300 bg-slate-100 text-base text-slate-600">
    ...
    <div class="bg-white border border-slate-300 rounded-lg p-6">
        <a
            ...
            class="inline-block rounded bg-blue-600 text-white px-6 py-2.5 text-base font-medium hover:bg-blue-700"
        >
