...
<div class="space-y-6">
    <div class="grid sm:grid-cols-2 gap-4">
        ...
        <x-form.readonly-field label='申請者名 <span class="text-sm text-slate-500">(自動取得)</span>' :value="$user->displayName()" />
        <x-form.readonly-field label='申請者(ID) <span class="text-sm text-slate-500">(自動取得)</span>' :value="$user->employee_id ?? '—'" />
    </div>
    ...
    @elseif ($isPurchasedType)
        <div class="max-w-md">
            <label for="purchase_site" class="block text-base mb-1">購入店舗名 <span class="text-red-600">*</span></label>
            <input id="purchase_site" name="purchase_site" value="{{ $value('purchase_site') }}" class="w-full rounded border border-slate-300 px-3 py-2 text-base" placeholder="例：〇〇ホームセンター 梅田店">
            @include('partials.field-error', ['field' => 'purchase_site'])
        </div>
    @else
        <div class="max-w-md">
            <label for="purchase_site" class="block text-base mb-1">購入サイト <span class="text-red-600">*</span></label>
            <select id="purchase_site" name="purchase_site" class="w-full rounded border border-slate-300 px-3 py-2 text-base">
    ...
    <div>
        <label @if (! $readonly) for="purchase_site_url" @endif class="block text-base mb-1">
            購入サイト URL
            @unless ($readonly)
                @if ($isPurchasedType)
                    <span class="text-sm text-slate-500">(任意入力)</span>
                @else
        ...
        @if ($readonly)
            @if ($application->purchase_site_url)
                <p class="rounded border border-slate-200 bg-slate-50 px-3 py-2 text-base break-all">
        ...
        @else
            @unless ($isPurchasedType)
                <p class="mb-2 text-base text-slate-800">「適格請求書（インボイス）」が発行可能なページのURLを入力してください。</p>
            @endunless
            <input id="purchase_site_url" type="url" name="purchase_site_url" value="{{ $value('purchase_site_url') }}" placeholder="https://" class="w-full rounded border border-slate-300 px-3 py-2 text-base">
            @include('partials.field-error', ['field' => 'purchase_site_url'])
        @endif
    </div>

    <div>
        <label @if (! $readonly) for="product_name" @endif class="block text-base mb-1">購入商品名 @unless ($readonly)<span class="text-red-600">*</span>@endunless</label>
        @if ($readonly)
            <x-form.readonly-field :value="$application->product_name" />
        @else
            <input id="product_name" name="product_name" value="{{ $value('product_name') }}" class="w-full rounded border border-slate-300 px-3 py-2 text-base">
    ...
    <div class="grid sm:grid-cols-2 gap-4">
        <div>
            <label @if (! $readonly) for="size" @endif class="block text-base mb-1">サイズ <span class="text-sm text-slate-500">(任意入力)</span></label>
            @if ($readonly)
                <x-form.readonly-field :value="$application->size ?: '—'" />
            @else
                <input id="size" name="size" value="{{ $value('size') }}" class="w-full rounded border border-slate-300 px-3 py-2 text-base">
            @endif
        </div>
        <div>
            <label @if (! $readonly) for="color_model" @endif class="block text-base mb-1">カラー/型式 <span class="text-sm text-slate-500">(任意入力)</span></label>
            @if ($readonly)
                <x-form.readonly-field :value="$application->color_model ?: '—'" />
            @else
                <input id="color_model" name="color_model" value="{{ $value('color_model') }}" class="w-full rounded border border-slate-300 px-3 py-2 text-base">
            @endif
        </div>
    </div>
    ...
    <div class="space-y-4">
        <div class="grid sm:grid-cols-[8rem_minmax(0,1fr)] gap-4 items-start max-w-3xl">
            <div>
                <label @if (! $readonly) for="quantity" @endif class="block text-base mb-1">数量 @unless ($readonly)<span class="text-red-600">*</span>@endunless</label>
                @if ($readonly)
                    <x-form.readonly-field :value="$application->quantity" />
                @else
                    <input id="quantity" type="number" name="quantity" min="1" value="{{ $value('quantity', 1) }}" class="w-full rounded border border-slate-300 px-3 py-2 text-base">
                    @include('partials.field-error', ['field' => 'quantity'])
                @endif
            </div>
            <div class="min-w-0">
                <label @if (! $readonly) for="price_including_tax" @endif class="block text-base mb-1">価格（税込） @unless ($readonly)<span class="text-red-600">*</span>@endunless</label>
                @if ($readonly)
                    <x-form.readonly-field :value="number_format($application->price_including_tax).'円'" />
                @else
                    <input id="price_including_tax" type="number" name="price_including_tax" min="1" value="{{ $value('price_including_tax') }}" class="w-full max-w-xs rounded border border-slate-300 px-3 py-2 text-base">
                    @error('price_including_tax')
                        <p class="text-base text-red-600 mt-1 whitespace-nowrap">{{ $message }}</p>
                    @enderror
                @endif
            </div>
        </div>
        <div>
            <label @if (! $readonly) for="remarks" @endif class="block text-base mb-1">備考</label>
            @if ($readonly)
                <x-form.readonly-field :value="$application->remarks ?: '—'" class="min-h-[5rem] whitespace-pre-wrap" />
            @else
                <textarea id="remarks" name="remarks" rows="3" class="w-full rounded border border-slate-300 px-3 py-2 text-base">{{ $value('remarks') }}</textarea>
    ...
    <div>
        <label @if (! $readonly) for="purchase_reason" @endif class="block text-base mb-1">購入理由 @unless ($readonly)<span class="text-red-600">*</span>@endunless</label>
        @if ($readonly)
            <x-form.readonly-field :value="$application->purchase_reason" class="min-h-[6rem] whitespace-pre-wrap" />
        @else
            <textarea id="purchase_reason" name="purchase_reason" rows="4" class="w-full rounded border border-slate-300 px-3 py-2 text-base">{{ $value('purchase_reason') }}</textarea>
    ...
    <div class="space-y-4">
        ...
        @else
            <div>
                <label for="item_destination" class="block text-base mb-1">備品の利用先 <span class="text-red-600">*</span></label>
                <select id="item_destination" name="item_destination" class="w-full max-w-md rounded border border-slate-300 px-3 py-2 text-base" data-conditional-trigger>
                    <option value="">選択してください</option>
            ...
            <div id="department-wrap" class="hidden">
                <label for="department" class="block text-base mb-1">
                    部 <span id="department-required-mark" class="text-red-600">*</span>
                </label>
                <input id="department" name="department" value="{{ $value('department') }}" class="w-full max-w-md rounded border border-slate-300 px-3 py-2 text-base">
                @include('partials.field-error', ['field' => 'department'])
            </div>

            <div id="section-wrap" class="hidden">
                <p id="section-only-hint" class="text-sm text-slate-500 mb-2 hidden">部を入力済みの場合、課は任意です。</p>
                <label for="section" class="block text-base mb-1">
                    課 <span id="section-required-mark" class="text-red-600 hidden">*</span>
                </label>
                <input id="section" name="section" value="{{ $value('section') }}" class="w-full max-w-md rounded border border-slate-300 px-3 py-2 text-base">
                @include('partials.field-error', ['field' => 'section'])
            </div>

            <div id="onsite-name-wrap" class="hidden">
                <label for="onsite_name" class="block text-base mb-1">
                    現場名 <span class="text-red-600">*</span>
                </label>
                <input id="onsite_name" name="onsite_name" value="{{ $value('onsite_name') }}" class="w-full max-w-md rounded border border-slate-300 px-3 py-2 text-base" placeholder="例：○○マンション新築工事">
    ...
    <div class="space-y-4">
        ...
        @else
            <div>
                <label for="delivery_destination" class="block text-base mb-1">
                    備品の届先
                    @if ($isPurchasedType)
                        <span class="text-sm text-slate-500">(任意入力)</span>
                    @else
                ...
                <select id="delivery_destination" name="delivery_destination" class="w-full max-w-md rounded border border-slate-300 px-3 py-2 text-base" data-conditional-trigger>
                    <option value="">選択してください</option>
            ...
            <div id="delivery-other-wrap" class="hidden space-y-4">
                <div class="grid sm:grid-cols-2 gap-4">
                    <div>
                        <label for="delivery_zip" class="block text-base mb-1">
                            お届け先〒
                            @unless ($isPurchasedType)<span class="text-red-600">*</span>@endunless
                        </label>
                        <input id="delivery_zip" name="delivery_zip" value="{{ $value('delivery_zip') }}" class="w-full rounded border border-slate-300 px-3 py-2 text-base">
                        @include('partials.field-error', ['field' => 'delivery_zip'])
                    </div>
                    <div>
                        <label for="delivery_recipient_name" class="block text-base mb-1">
                            受取人氏名
                            @unless ($isPurchasedType)<span class="text-red-600">*</span>@endunless
                        </label>
                        <input id="delivery_recipient_name" name="delivery_recipient_name" value="{{ $value('delivery_recipient_name') }}" class="w-full rounded border border-slate-300 px-3 py-2 text-base">
                        @include('partials.field-error', ['field' => 'delivery_recipient_name'])
                    </div>
                </div>
                <div>
                    <label for="delivery_address" class="block text-base mb-1">
                        お届け先
                        @unless ($isPurchasedType)<span class="text-red-600">*</span>@endunless
                    </label>
                    <input id="delivery_address" name="delivery_address" value="{{ $value('delivery_address') }}" class="w-full rounded border border-slate-300 px-3 py-2 text-base">
                    @include('partials.field-error', ['field' => 'delivery_address'])
                </div>
                <div class="max-w-md">
                    <label for="delivery_recipient_phone" class="block text-base mb-1">
                        受取人電話
                        @unless ($isPurchasedType)<span class="text-red-600">*</span>@endunless
                    </label>
                    <input id="delivery_recipient_phone" name="delivery_recipient_phone" value="{{ $value('delivery_recipient_phone') }}" class="w-full rounded border border-slate-300 px-3 py-2 text-base">
    ...
    <div>
        <p class="block text-base mb-2">
            購入希望日
            @unless ($readonly)
                @if ($isPurchasedType)
                    <span class="text-sm text-slate-500">(任意入力)</span>
                @else
                    <span class="text-red-600">*</span><span class="text-sm text-slate-600">＊数日かかります</span>
                @endif
            @endunless
        </p>
        @if ($readonly)
        ...
        @else
            <div class="space-y-2">
                @foreach (EPA::PURCHASE_URGENCIES as $key => $label)
                    <label class="flex items-start gap-2 text-base cursor-pointer">
                        <input
                            type="radio"
