...
<div class="rounded-lg border-2 border-blue-400 bg-blue-50/30 p-4 space-y-4">
    <h2 class="font-bold text-base">承認</h2>
    ...
    @if ($application?->hasFirstApproval())
        <div class="rounded border border-slate-200 bg-white p-3 space-y-2">
            <h3 class="text-base font-semibold text-slate-700">1次承認（部長）</h3>
            <div class="grid sm:grid-cols-2 gap-3 text-base">
                <div>
                    <p class="text-slate-500 text-sm mb-1">日時</p>
                    <p>{{ $application->firstApprovedAtDisplay() ?? '—' }}</p>
                </div>
                <div>
                    <p class="text-slate-500 text-sm mb-1">確認者</p>
                    <p>{{ $storedFirstConfirmer ?: '—' }}</p>
                </div>
            </div>
            <div>
                <p class="text-slate-500 text-sm mb-1">判定</p>
                <p class="font-medium">{{ $application->firstApprovalDecisionLabel() ?: '—' }}</p>
            </div>
            @if ($application->first_approval_memo)
                <div>
                    <p class="text-slate-500 text-sm mb-1">メモ</p>
    ...
    @if ($isSecondStage || ($readonly && $application?->status !== EquipmentPurchaseApplication::STATUS_PENDING))
        <div class="space-y-4 {{ $application?->hasFirstApproval() ? 'pt-2 border-t border-slate-200' : '' }}">
            @if ($application?->hasFirstApproval())
                <h3 class="text-base font-semibold text-slate-700">2次承認（支店長）</h3>
            @endif

            <div class="grid sm:grid-cols-2 gap-4">
                <div>
                    @if ($readonly)
                        <label class="block text-base mb-1">Datetime <span class="text-sm text-slate-500">(日本時間)</span></label>
                        <p class="rounded border border-slate-200 bg-slate-50 px-3 py-2 text-base">
                            {{ $application->approvedAtDisplay() ?? '—' }}
                        </p>
                    @else
                        <label for="approved_at" class="block text-base mb-1">Datetime <span class="text-sm text-slate-500">(日本時間)</span></label>
                        <input
                            id="approved_at"
                            type="datetime-local"
                            name="approved_at"
                ...
                <div>
                    <label class="block text-base mb-1">確認者 @unless ($readonly)<span class="text-sm text-slate-500">{{ $isGlobalApprover ? '(入力)' : '(自動取得)' }}</span>@endunless</label>
                    @if ($readonly)
                        <p class="rounded border border-slate-200 bg-slate-50 px-3 py-2 text-base">
            ...
            <div>
                @if ($readonly)
                    <p class="block text-base mb-2">承認</p>
                    <p class="text-base font-medium {{ $application->approvalStatusColorClass() }}">
                        {{ $application->approvalSummaryText() }}
                    </p>
                @else
                    <p class="block text-base mb-2">承認 <span class="text-red-600">*</span></p>
            ...
            <div>
                @if ($readonly)
                    <label class="block text-base mb-1">メモ</label>
                    <p class="rounded border border-slate-200 bg-slate-50 px-3 py-2 text-base min-h-[4rem] whitespace-pre-wrap">{{ $application->approval_memo ?: '—' }}</p>
                @else
                    <label for="approval_memo" class="block text-base mb-1">メモ</label>
