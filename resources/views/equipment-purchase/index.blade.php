...
<div class="max-w-3xl mx-auto">
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h1 class="text-2xl font-bold">備品購入</h1>
        <div class="flex flex-wrap gap-x-4 gap-y-1 text-base">
    ...
    <div class="equipment-purchase-index-panel">
        ...
        @if (! ($canSubmit ?? true))
            <div class="mb-4 rounded border border-amber-200 bg-amber-50 px-4 py-3 text-base text-amber-900">
        ...
        <p class="text-center text-lg font-medium mb-6">備品購入タイプを選択してください</p>
        ...
        <div class="space-y-4 @if (! ($canSubmit ?? true)) opacity-50 pointer-events-none @endif">
            @foreach (\App\Models\EquipmentPurchaseApplication::TYPE_GROUPS as $group)
                <div class="equipment-purchase-type-row">
                    <div class="text-base font-medium text-slate-700 leading-snug">{{ $group['label'] }}</div>
        ...
        <div class="equipment-purchase-catalog">
            ...
            >
                よく買う商品一覧
                <span class="mt-0.5 block text-sm font-normal text-blue-600">Google スプレッドシート</span>
            </a>
        </div>
    </div>
</div>
